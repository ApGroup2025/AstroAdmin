...
import React, { useEffect, useState } from "react";
import { getAllPoojas } from "../constants/api";
import { deletePoojaById } from "../constants/api";
...
export default function AllPoojas() {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await deletePoojaById(id);
      alert("Pooja deleted successfully");
      setPoojas((prev) => prev.filter((pooja) => pooja._id !== id)); 
    } catch (err) {
      console.error("Delete failed:", err);
      alert(err.response?.data?.message || "Failed to delete pooja");
    }
  };

  const handleSeeMore = () => setVisibleCount((prev) => prev + 8);
  const handleSeeLess = () => setVisibleCount(8);

  const poojasToShow = poojas.slice(0, visibleCount);

  if (loading) return <p className="text-center mt-10">Loading poojas...</p>;
        ...
        <>
          <div className="space-y-4">
            {poojasToShow.map((pooja) => (
              <div
                key={pooja._id || pooja.id}
                className="bg-white shadow-md rounded-xl p-4 border border-gray-200 hover:shadow-lg transition flex flex-col md:flex-row items-start md:items-center justify-between"
              >
                ...
                <button
                  ...
                  className="mt-3 md:mt-0 md:ml-4 px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition"
                >
                  Delete
                </button>
              </div>
